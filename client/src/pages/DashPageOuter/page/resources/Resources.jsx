...
const ResourcesO = () => {
  ...
  const getRequests = async (values) =>{
    ...
    const res = await fetch(`${baseurl}/organisation/getOrganisation/${cookies.get('_id')}`,{
      method:"GET",
      headers:{
    ...
    let hmm = 0
    setResources(jack.data.doc.resources.map((session)=>{hmm += 1;return {...session, id : hmm}}))
    console.log(resources, "gghgj", jack.data.doc.resources.map((session)=>{hmm += 1;return {...session, id : hmm}}))
